...
class CacheManager
{
    ...
    public function remove(string $path): void
    {
        ...
        // Group filters by resolver to avoid calling remove() multiple times on the same resolver for the same filter
        foreach ($filters as $filter) {
            $resolver = $this->getResolver($filter, null);
            $resolver->remove($path, $filter, $prefix);
        }
    }
    ...
    private function getResolver(string $filter, string|null $resolver): ResolverInterface
    {
        ...
        if (!isset($this->resolvers[$resolverName])) {
            throw new OutOfBoundsException(\sprintf(
                'Could not find resolver "%s" for "%s" filter type. Available resolvers: [%s]',
                $resolverName,
                $filter,
