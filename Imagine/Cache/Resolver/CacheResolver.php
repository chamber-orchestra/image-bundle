...
//        }
    ...
    protected function removePathAndFilter(string $path, string $filter): void
    {
        ...
        if (!$this->cache->hasItem($indexKey)) {
            return;
        }

        $index = $this->cache->getItem($indexKey);
        $index = $index->get();

        if (null === $path) {
            $this->cache->deleteItems($index);
            $index = [];
        } else {
    ...
    protected function saveToCache(string $cacheKey, $content): bool
    {
        ...
        $item = $this->cache->getItem($indexKey);
        $item->set($index)->expiresAfter($this->options['lifetime']);
        if ($this->cache->save($item)) {
            $item = $this->cache->getItem($cacheKey);
            $item->set($content)->expiresAfter($this->options['lifetime']);
